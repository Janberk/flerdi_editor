...
define([ "jquery", 'network' ], (function($, Network) {
	 ...
	 *            boolean, tru, alle controller and views will be created , to
	 *            show this network on the drawArea, if false, then not
	 */
	Network_Organisation.prototype.newNetwork = function(network,
		...
		if (drawOnDrawArea) {
			for ( var i = 0; i < this.networks.networkElements.length; i++) {
				controllerFactory.build(this.networks.networkElements[i],
						"draw_area");
			}
