...
define([ 'jquery' ], function($) {
	var Menubar = function() {
		$('#menubar').addClass('navbar navbar-static-top').append(
				$(document.createElement('div')).addClass('navbar-inner')
						.append(
								$(document.createElement('div')).addClass(
										'container pull-left').append(
										$(document.createElement('ul'))
												.addClass('nav'))).append(
								$(document.createElement('div')).addClass(
										'pull-right').append(
										$(document.createElement('div'))
												.addClass('brand').append(
														'Flerdi'))));
	};
	Menubar.prototype.addMenu = function(title) {
		$('#menubar .nav').append(
				$(document.createElement('li')).on('mouseout', function() {
					if ($('#menubar').find($(this)).length == 0)
						active = false;
				}).attr('name', title).addClass('dropdown').append(
						$(document.createElement('button')).addClass(
								'btn btn-link').append(title)).hover(
						function() {
							$(this).addClass('open')
						}, function() {
	...
	Menubar.prototype.addSubMenu = function(menu, subtitle, funct) {
		if ($('#menubar .nav li[name=' + menu + '] ul').length == 0) {
			$('#menubar .nav li[name=' + menu + ']').append(
		...
		$('#menubar .nav li[name=' + menu + '] ul').append(
				$(document.createElement('li')).append(
						$(document.createElement('a'))
						.attr({
							'id' : 'btn-' + subtitle,
							'href' : '#'
						}).append(subtitle).on('click', funct || function() {
